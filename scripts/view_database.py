...
from src.database.models import (ActivityLog, ActivityPattern, ActivitySession,
...
def view_recent_activities(limit=10):
    ...
    try:
        activities = (
            ...
            .order_by(ActivityLog.timestamp_start.desc())
            .limit(limit)
            .all()
        )
        ...
        for activity in activities:
            ...
            print(
                f"{activity.timestamp_start.strftime('%Y-%m-%d %H:%M')} | "
...
def view_sessions(limit=10):
    ...
    try:
        ...
        for sess in sessions:
            ...
            print(
                f"{sess.start_time.strftime('%Y-%m-%d %H:%M')} | "
...
def view_patterns():
    ...
    try:
        patterns = (
            session.query(ActivityPattern)
            .order_by(ActivityPattern.usage_count.desc())
            .all()
        )
        ...
        for pattern in patterns:
            print(
                f"{pattern.pattern_name:30s} | {pattern.session_type:12s} | "
...
def view_database_stats():
    ...
    try:
        ...
        recent_activities = (
            session.query(ActivityLog)
            .filter(ActivityLog.timestamp_start >= recent_cutoff)
            .count()
        )
