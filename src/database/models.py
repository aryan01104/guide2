...
class ActivityLog(Base):
    ...
    session_id = Column(
        Integer, ForeignKey("activity_sessions.id"), nullable=True, index=True
    )
    timestamp_start = Column(DateTime, nullable=False, index=True)
    event_type = Column(
        String(50), nullable=False
    )  # app_session or browser_tab_session
    ...
    classification_text = Column(
        Text, nullable=True
    )  # Full LLM classification response with reasoning
...
class CustomClassification(Base):
    ...
    classification = Column(
        String(50), nullable=False
    )  # productive, unproductive, unclear
    created_at = Column(DateTime, default=datetime.utcnow)
    updated_at = Column(DateTime, default=datetime.utcnow, onupdate=datetime.utcnow)
...
class ActivitySession(Base):
    ...
    )  # "Web Development", "Entertainment Break"
    productivity_score = Column(
        Integer, nullable=True
    )  # -50 to +50 time-weighted average of activities
