...
from ..database.operations import (assign_session_to_activities,
                                   get_db_session, save_activity_session)
from .flow_analysis import (ActivityLogStub, batch_sessionize, session_name,
                            weighted_score)
...
def fetch_activity_logs():
    ...
    try:
        rows = (
            ...
            .filter(ActivityLog.duration_sec > 0)
...
def analyze_and_group_activities():
    ...
    print(
        f"[BATCH SESSIONIZER] Grouped {len(activities)} logs into {len(sessions)} sessions."
    )
    return sessions


def sessionize_orphan_logs():
    """
    ...
    """
    from ..database.operations import (fetch_activities_in_time_range,
    ...
    for start_time, end_time in ranges:
        print(
            f"[BOUNDED_SESSIONIZER] Processing range: {start_time.strftime('%Y-%m-%d %H:%M')} to {end_time.strftime('%Y-%m-%d %H:%M')}"
        )
        ...
        for sess in sessions:
            ...
            has_unsessionized = any(
                hasattr(a, "session_id") and a.session_id is None for a in sess
            )
            ...
            if has_unsessionized or not hasattr(sess[0], "session_id"):
                ...
                score = weighted_score(sess)
                start = sess[0].timestamp_start
                end = sess[-1].timestamp_start + timedelta(
                    seconds=sess[-1].duration_sec
                )
                total_duration = sum(a.duration_sec for a in sess)
                ...
                all_created_sessions.append(
                    {"session_id": session_id, "start": start, "end": end}
                )
                total_sessions_created += 1

    print(
        f"[BOUNDED_SESSIONIZER] Created {total_sessions_created} sessions from {len(ranges)} time ranges"
    )
